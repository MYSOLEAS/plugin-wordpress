 ...
 * @return array
 */
function Soleaspay_Add_To_Woocommerce(array $methods): array
{
    $methods[] = 'Soleaspay_Woocommerce_Gateway';
    return $methods;
}
add_filter('woocommerce_payment_gateways', 'Soleaspay_Add_To_Woocommerce');

/**
 * return callback function to WooCommerce
 */
function init_soleaspay_gateway ()
{
    class Soleaspay_Woocommerce_gateway extends WC_Payment_Gateway {
        ...
        public function __construct()
        {
            ...
            */
            ...
            $this->method_title = __('SoleasPay Payment Gateway for Woocommerce', $this->id);
            ...
            */
            ...
            add_action('woocommerce_after_checkout_form', [$this, 'soleaspay_payment_script']);
            add_action('woocommerce_thankyou', [$this, 'soleaspay_thankyou_page']);
        }
        ...
        public function init_form_fields()
        {
            $this->form_fields = [
                ...
                'apiKey' => [
                    ...
                    'type' => 'text',
                    'description' => __('Copy and place the ApiKey SoleasPay for Have Acces to use this Payment. If you have not the ApiKey, please contact the administrator', $this->id),
                ...
                'currency' => [
                    'title' => __('Soleaspay currency', $this->id),
                    'type' => 'select',
        ...
        private function soleaspay_generate_form(array $content): string
        {
            ...
            $html .= "<form name='soleaspay_data_form' id='sopay_data_form' method='post' action='{$action}'>";
            foreach ($content as $name => $value) {
                $html .= "<input type='hidden' name='{$name}' value='{$value}' readonly>";
            }
            $html.= "</form>";
            $script = "<script>document.getElementById('soleaspay_data_form').submit()</script>";
            $html .= $script;
            $html .= "</div>";
            return $html;
        }
         ...
         * @return string
         */
        private function soleaspay_generate_scripts(): string
        {
        ...
        public function process_payment($order_id): array
        {
            ...
            try{
                ...
                $html = $this->soleaspay_generate_form($options);
                ...
                */
                ...
                $order->add_order_note("SoleasPay payment init failed with message: " . $ex->getMessage());
        ...
        public function soleaspay_payment_script()
        {
            if($this->enabled === 'no')
                return;
            ...
            if (is_checkout()){ 
                ...
                */
                wc_enqueue_js($this->soleaspay_generate_scripts());
            }
        }

        public function soleaspay_thankyou_page($order_id)
        {
            $order = wc_get_order($order_id);
 ...
 */
add_action('plugins_loaded', 'init_soleaspay_gateway');